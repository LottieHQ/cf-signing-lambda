...
def signed_request(request):
    ...
    # Grab the region from the lambda function URL, ex: 7zfl3o6wyka0ejoqe.lambda-url.eu-west-2.on.aws
    url_segments = urllib.parse.urlparse(url).netloc.split('.')
    region = url_segments[2]
    req = AWSRequest(method=method, url=url, params=params, headers=headers, data=data)
